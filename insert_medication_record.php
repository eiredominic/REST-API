<?php  
	...
	if ($num_rows > 0) {
		$result = mysqli_query($db->myconn, "INSERT INTO medication(minderid, childid, medication, amount, date, time) VALUES('$minderid', '$childid', '$medication', '$amount', '$date', '$time')");  

		// check if row inserted or not  
		if ($result) {  
			$response["success_msg"] = 0;  
			$response["message"] = "Medication record successfully inserted";  
