<?php  
	...
	if ($num_rows > 0) {
		...
		$result = mysqli_query($db->myconn, "INSERT INTO changing(minderid, childid, type, date, time) VALUES('$minderid', '$childid', '$type', '$date', '$time')");  

		// check if row inserted or not  
		if ($result) {  
			$response["success_msg"] = 0;  
			$response["message"] = "Changing record successfully inserted";  
