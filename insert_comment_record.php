<?php  
	...
	if ($num_rows > 0) {
		$result = mysqli_query($db->myconn, "INSERT INTO comments(minderid, childid, comment, date, time) VALUES('$minderid', '$childid', '$comment', '$date', '$time')");  

		// check if row inserted or not  
		if ($result) {  
			$response["success_msg"] = 0;  
			$response["message"] = "Comment record successfully inserted";  
