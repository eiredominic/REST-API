<?php  
	...
	if ($num_rows > 0) {	
		$result = mysqli_query($db->myconn, "INSERT INTO sleep(minderid, childid, amount, date, time) VALUES('$minderid', '$childid', '$amount', '$date', '$time')");  

		// check if row inserted or not  
		if ($result) {  
			$response["success_msg"] = 0;  
			$response["message"] = "Sleeping record successfully inserted";  
